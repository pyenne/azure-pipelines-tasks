...
export function convertJobName(jobName: string): string {
  let includeJob: boolean = jobName.includes("/job/");
  if (!includeJob) {
    return "/job/" + jobName.replace(/\//g, "/job/");
  } else {
    let newJobName: string = jobName.toLowerCase().replace(/\/job\//g, "/");
    return "/job/" + newJobName.replace(/\//g, "/job/");
  }
}
...
export function isPipelineJob(
  ...
  request.get(
    { url: wfapiUrl, strictSSL: taskOptions.strictSSL },
    (err, response, body) => {
...
export function pollCreateRootJob(
  queueUri: string,
  jobQueue: JobQueue,
  taskOptions: TaskOptions
): Q.Promise<Job> {
  ...
  const poll = async () => {
    await createRootJob(queueUri, jobQueue, taskOptions)
      .then((job: Job) => {
        if (job != null) {
          defer.resolve(job);
        } else {
...
function createRootJob(
  ...
  request
    .get(
      { url: queueUri, strictSSL: taskOptions.strictSSL },
      function requestCallback(err, httpResponse, body) {
        ...
        if (err) {
          ...
          defer.reject(
            new HttpError(
              httpResponse,
              "Job progress tracking failed to read job queue"
            )
          );
        } else {
          const parsedBody: any = JSON.parse(body);
          tl.debug(
            `parsedBody for: ${queueUri} : ${JSON.stringify(parsedBody)}`
          );
          ...
          if (parsedBody.cancelled || parsedBody.canceled) {
            ...
            if (!executable) {
              ...
              const rootJob: Job = new Job(
                jobQueue,
                null,
                parsedBody.task.url,
                parsedBody.executable.url,
                parsedBody.executable.number,
...
export function pollSubmitJob(taskOptions: TaskOptions): Q.Promise<string> {
  ...
  const poll = async () => {
    await getCrumb(taskOptions)
      .then(async (crumb: string) => {
        if (crumb != null) {
          await submitJob(taskOptions)
            .then((queueUri: string) => {
              if (queueUri != null) {
    ...
    .post(
      ...
      function teamBuildRequestCallback(err, httpResponse, body) {
        ...
        if (err) {
          ...
          // team-build plugin endpoint failed because it is not installed
          console.log(
          ...
          const jobQueuePostData: any = addCrumb(
            taskOptions.parameterizedJob
              ? {
          ...
          request
            .post(
              jobQueuePostData,
              function jobQueueRequestCallback(err, httpResponse, body) {
                tl.debug(
                  "submitJob().jobQueueRequestCallback(jobQueuePostData)"
                );
                if (err) {
                  if (err.code == "ECONNRESET") {
                    tl.debug(err);
                  ...
                  defer.reject(
                    new HttpError(httpResponse, "Job creation failed.")
                  );
                } else {
                  const queueUri: string = addUrlSegment(
...
function getCrumb(taskOptions: TaskOptions): Q.Promise<string> {
  const defer: Q.Deferred<string> = Q.defer<string>();
  const crumbRequestUrl: string = addUrlSegment(
    taskOptions.serverEndpointUrl,
  ...
  request
    .get(
      { url: crumbRequestUrl, strictSSL: taskOptions.strictSSL },
...
function parseJobParameters(jobParameters: string[]): any {
  ...
  for (let i: number = 0; i < jobParameters.length; i++) {
    ...
    if (splitIndex <= 0) {
      // either no paramValue (-1), or no paramName (0)
      throw (
        'Job parameters should be specified as "parameterName=parameterValue" with one name, value pair per line. Invalid parameter line: ' +
        jobParameters[i]
      );
    }
...
function getTeamParameters(taskOptions: TaskOptions): any {
  ...
  if (taskOptions.isMultibranchPipelineJob) {
    formData["QueueJobTask.MultibranchPipelineBranch"] =
      taskOptions.multibranchPipelineBranch;
  }

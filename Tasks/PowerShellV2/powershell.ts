...
function getActionPreference(
  vstsInputName: string,
  ...
  if (
    validActions
      .map((actionPreference) => actionPreference.toUpperCase())
      .indexOf(result.toUpperCase()) < 0
  ) {
    throw new Error(
      tl.loc(
        "JS_InvalidActionPreference",
        vstsInputName,
        result,
        validActions.join(", ")
      )
...
async function run() {
  try {
    ...
    // Get inputs.
    let input_errorActionPreference: string = getActionPreference(
      "errorActionPreference",
      "Stop"
    );
    let input_warningPreference: string = getActionPreference(
      "warningPreference",
      "Default"
    );
    let input_informationPreference: string = getActionPreference(
      "informationPreference",
      "Default"
    );
    let input_verbosePreference: string = getActionPreference(
      "verbosePreference",
      "Default"
    );
    let input_debugPreference: string = getActionPreference(
      "debugPreference",
      "Default"
    );
    ...
    let input_workingDirectory = tl.getPathInput(
      "workingDirectory",
      /*required*/ true,
      /*check*/ true
    );
    ...
    if (input_targetType.toUpperCase() == "FILEPATH") {
      ...
      if (
        !tl.stats(input_filePath).isFile() ||
        !input_filePath.toUpperCase().match(/\.PS1$/)
      ) {
    ...
    const input_runScriptInSeparateScope = tl.getBoolInput(
    ...
    if (input_errorActionPreference.toUpperCase() != "DEFAULT") {
      contents.push(
        `$ErrorActionPreference = '${input_errorActionPreference}'`
      );
    }
    ...
    if (input_informationPreference.toUpperCase() != "DEFAULT") {
      contents.push(
        `$InformationPreference = '${input_informationPreference}'`
      );
    }
    ...
    let script = "";
    if (input_targetType.toUpperCase() == "FILEPATH") {
      script = `. '${input_filePath.replace(
        /'/g,
        "''"
      )}' ${input_arguments}`.trim();
    } else {
      script = `${input_script}`;
    }
    ...
    tl.logDetail(
      uuidV4(),
      tl.loc("JS_FormattedCommand", script),
      null,
      "command",
      "command",
      0
    ...
    let powershell = tl
      .tool(
        tl.which("pwsh") || tl.which("powershell") || tl.which("pwsh", true)
      )
      .arg("-NoLogo")
      .arg("-NoProfile")
      .arg("-NonInteractive")
